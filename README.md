...
3. For every monitorable device/client it creates a **ping monitor** inside the matching Kuma group — using the device's management IP: its UniFi **Fixed IP** (the "IP Address" override field / DHCP reservation) if one is set, otherwise its currently-observed live IP, falling back to the last known one if it isn't connected right now. Fixed IP wins on purpose — for a multi-homed device (one MAC, several VLANs via sub-interfaces) the live IP UniFi reports is just whichever network it currently considers "primary," while a Fixed IP is the admin's explicit choice, and the two can legitimately differ while both stay reachable. Set one in the UniFi client's details page if you want to pin monitoring to a specific VLAN/address on a multi-homed box. If a monitor already exists for that device, its full configuration (hostname, interval, retries, active status) is reconciled to match every cycle — so an IP change in UniFi (e.g. a DHCP lease renewal, or setting/changing a Fixed IP) reaches an already-synced monitor instead of leaving it pointed at a stale address forever. This means any manual edit you make to a managed monitor directly in the Kuma UI (e.g. changing its interval) will be reverted on the next sync — managed monitors are meant to be fully owned by unifi-kuma's config, not hand-tuned.
